...
open class PiloterrAlertDialog(context: Context) : AlertDialog(context, R.style.PiloterrAlertDialogTheme) {

    var isCelebratory: Boolean = false
    private val view: RelativeLayout = LayoutInflater.from(context).inflate(R.layout.dialog_piloterr_base, null) as RelativeLayout
    ...
    fun addButton(stringRes: Int, isPrimary: Boolean, isDestructive: Boolean = false, function: ((PiloterrAlertDialog, Int) -> Unit)? = null): Button {
        return addButton(context.getString(stringRes), isPrimary, isDestructive, true, function)
    }

    fun addButton(string: String, isPrimary: Boolean, isDestructive: Boolean = false, autoDismiss: Boolean = true, function: ((PiloterrAlertDialog, Int) -> Unit)? = null): Button {
        val button: Button = if (isPrimary) {
            if (isDestructive) {
                buttonsWrapper.inflate(R.layout.dialog_piloterr_primary_destructive_button) as? Button
            } else {
                buttonsWrapper.inflate(R.layout.dialog_piloterr_primary_button) as? Button
            }
        } else {
            val button = buttonsWrapper.inflate(R.layout.dialog_piloterr_secondary_button) as? Button
            if (isDestructive) {
    ...
    fun addButton(buttonView: View, autoDismiss: Boolean = true, function: ((PiloterrAlertDialog, Int) -> Unit)? = null): View {
        val weakThis = WeakReference<PiloterrAlertDialog>(this)
    ...
    companion object {
        private var dialogQueue = mutableListOf<PiloterrAlertDialog>()

        private fun showNextInQueue(currentDialog: PiloterrAlertDialog) {
        ...
        private fun addToQueue(dialog: PiloterrAlertDialog) {
