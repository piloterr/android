...
import com.piloterr.android.piloterr.databinding.FragmentRefreshRecyclerviewBinding
...
import com.piloterr.android.piloterr.ui.adapter.tasks.*
...
import com.piloterr.android.piloterr.ui.viewHolders.tasks.BaseTaskViewHolder
    ...
    private fun handleTaskResult(result: TaskScoringResult, value: Int) {
        if (classType == Task.TYPE_REWARD) {
            (activity as? MainActivity)?.let { activity ->
                PiloterrSnackbar.showSnackbar(activity.snackbarContainer, null, getString(R.string.notification_purchase_reward),
                        BitmapDrawable(resources, PiloterrIconsHelper.imageOfGold()),
                        ContextCompat.getColor(activity, R.color.yellow_10),
                        "-$value",
                        PiloterrSnackbar.SnackbarDisplayType.DROP)
            }
        } else {
    ...
    protected fun showBrokenChallengeDialog(task: Task) {
        context?.let {
            ...
            taskRepository.getTasksForChallenge(task.challengeID).subscribe(Consumer { tasks ->
                ...
                val dialog = PiloterrAlertDialog(it)
