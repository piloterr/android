...
import com.piloterr.android.piloterr.models.inventory.QuestProgressCollect
...
import com.piloterr.android.piloterr.ui.views.*
...
class QuestProgressView : LinearLayout {
    ...
    private fun setupView(context: Context) {
        ...
        questImageCaretView.setImageBitmap(PiloterrIconsHelper.imageOfCaret(ContextCompat.getColor(context, R.color.white), true))
        ...
        pendingDamageIconView.setImageBitmap(PiloterrIconsHelper.imageOfDamage())
        bossHealthView.setSecondaryIcon(PiloterrIconsHelper.imageOfHeartDarkBg())
        bossRageView.setSecondaryIcon(PiloterrIconsHelper.imageOfRage())
    ...
    override fun onDraw(canvas: Canvas?) {
        if (quest?.isValid == true) {
            ...
            if (colors != null) {
                ...
                PiloterrIcons.drawQuestBackground(canvas, rect, colors.darkColor, colors.mediumColor, colors.extraLightColor)
    ...
    private fun setupRageStrikeViews() {
        ...
        progress?.rageStrikes?.sortedByDescending { it.wasHit }?.forEach { strike ->
            ...
            if (strike.wasHit) {
                DataBindingUtils.loadImage("rage_strike_${strike.key}") {
                    Observable.just(it)
                            ...
                            .subscribe(Consumer { bitmap ->
                                ...
                                iconView.setImageBitmap(PiloterrIconsHelper.imageOfRageStrikeActive(context, scaledImage))
                ...
                iconView.setImageBitmap(PiloterrIconsHelper.imageOfRageStrikeInactive())
...
//        alert.setSubtitle(R.string.strike_description_subtitle)
    ...
    private fun updateCaretImage() {
        if (questImageWrapper.visibility == View.VISIBLE) {
            questImageCaretView.setImageBitmap(PiloterrIconsHelper.imageOfCaret(quest?.colors?.extraLightColor ?: 0, true))
        } else {
            questImageCaretView.setImageBitmap(PiloterrIconsHelper.imageOfCaret(quest?.colors?.extraLightColor ?: 0, false))
        }
    }
}
