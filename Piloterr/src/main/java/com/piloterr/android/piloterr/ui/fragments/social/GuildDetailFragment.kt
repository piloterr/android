...
import com.piloterr.android.piloterr.MainNavDirections
...
import com.piloterr.android.piloterr.ui.views.PiloterrIcons
...
class GuildDetailFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        guildBankIconView.setImageBitmap(PiloterrIconsHelper.imageOfGem())
        ...
        joinGuildButton.setOnClickListener {
            viewModel?.joinGroup {
                ...
                if (activity != null) {
                    PiloterrSnackbar.showSnackbar(activity.snackbarContainer, getString(R.string.joined_guild), PiloterrSnackbar.SnackbarDisplayType.NORMAL)
                }
            }
        }
    ...
    private fun showLeaveSnackbar() {
        val activity = activity as? MainActivity
        if (activity != null) {
            PiloterrSnackbar.showSnackbar(activity.snackbarContainer, getString(R.string.left_guild), PiloterrSnackbar.SnackbarDisplayType.NORMAL)
        }
    }
    ...
    private fun updateGuild(guild: Group?) {
        ...
        guildMembersIconView.setImageBitmap(PiloterrIcons.imageOfGuildCrestMedium((guild?.memberCount ?: 0).toFloat()))
