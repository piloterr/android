...
import com.piloterr.android.piloterr.helpers.UserStatComputer
...
class StatsFragment: BaseMainFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        leftSparklesView.setImageBitmap(PiloterrIconsHelper.imageOfAttributeSparklesLeft())
        rightSparklesView.setImageBitmap(PiloterrIconsHelper.imageOfAttributeSparklesRight())
        context?.let {
            val color = it.getThemeColor(R.attr.colorPrimaryOffset)
            distributeEvenlyHelpButton.setImageBitmap(PiloterrIconsHelper.imageOfInfoIcon(color))
            distributeClassHelpButton.setImageBitmap(PiloterrIconsHelper.imageOfInfoIcon(color))
            distributeTaskHelpButton.setImageBitmap(PiloterrIconsHelper.imageOfInfoIcon(color))
        }
    ...
    private fun showBulkAllocateDialog() {
        context?.let { context ->
            val dialog = BulkAllocateStatsDialog(context, PiloterrBaseApplication.userComponent)
            dialog.show()
        }
    }
    ...
    private fun showHelpAlert(resourceId: Int) {
        val alert = context?.let { PiloterrAlertDialog(it) }
