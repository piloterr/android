...
import com.piloterr.android.piloterr.databinding.AvatarWithBarsBinding
import com.piloterr.android.piloterr.events.BoughtGemsEvent
...
class AvatarWithBarsViewModel(private val context: Context, private val binding: AvatarWithBarsBinding, userRepository: UserRepository? = null) {
    ...
    init {
        binding.hpBar.setIcon(PiloterrIconsHelper.imageOfHeartLightBg())
        binding.xpBar.setIcon(PiloterrIconsHelper.imageOfExperience())
        binding.mpBar.setIcon(PiloterrIconsHelper.imageOfMagic())
    ...
    companion object {
        ...
        private fun setUserLevelWithClass(context: Context, textView: TextView, level: Int?, userClassString: String, habitClass: String?) {
            ...
            when (habitClass) {
                Stats.WARRIOR -> drawable = BitmapDrawable(context.resources, PiloterrIconsHelper.imageOfWarriorDarkBg())
                Stats.ROGUE -> drawable = BitmapDrawable(context.resources, PiloterrIconsHelper.imageOfRogueDarkBg())
                Stats.MAGE -> drawable = BitmapDrawable(context.resources, PiloterrIconsHelper.imageOfMageDarkBg())
                Stats.HEALER -> drawable = BitmapDrawable(context.resources, PiloterrIconsHelper.imageOfHealerDarkBg())
            }
