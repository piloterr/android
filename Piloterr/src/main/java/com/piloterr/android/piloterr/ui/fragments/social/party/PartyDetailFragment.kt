...
import com.piloterr.android.piloterr.ui.helpers.*
import com.piloterr.android.piloterr.ui.viewHolders.GroupMemberViewHolder.GroupMemberViewHolder
import com.piloterr.android.piloterr.ui.viewmodels.PartyViewModel
...
import com.piloterr.android.piloterr.ui.views.social.InvitationsView
import com.piloterr.android.piloterr.ui.views.social.OldQuestProgressView
...
class PartyDetailFragment : BaseFragment() {
    ...
    private fun showSendMessageToUserDialog(userID: String, username: String) {
        ...
        val addMessageDialog = context?.let { PiloterrAlertDialog(it) }
            ...
            socialRepository.postPrivateMessage(userID, emojiEditText.text.toString())
                    .subscribe(Consumer {
                        (activity as? MainActivity)?.snackbarContainer?.let { it1 ->
                            PiloterrSnackbar.showSnackbar(it1,
                                    String.format(getString(R.string.profile_message_sent_to), username), PiloterrSnackbar.SnackbarDisplayType.NORMAL)
                        }
                    }, RxErrorHandler.handleEmptyError())
    ...
    private fun showTransferOwnerShipDialog(userID: String, displayName: String) {
        val dialog = context?.let { PiloterrAlertDialog(it) }
        dialog?.addButton(R.string.transfer, true) { _, _ ->
            socialRepository.transferGroupOwnership(viewModel?.groupID ?: "", userID)
                    .subscribe(Consumer {
                        (activity as? MainActivity)?.snackbarContainer?.let { it1 ->
                            PiloterrSnackbar.showSnackbar(it1,
                                    String.format(getString(R.string.transferred_ownership), displayName), PiloterrSnackbar.SnackbarDisplayType.NORMAL)
                        }
                    }, RxErrorHandler.handleEmptyError())
            activity?.dismissKeyboard()
        }
        ...
        dialog?.addButton(R.string.remove, true) { _, _ ->
            socialRepository.removeMemberFromGroup(viewModel?.groupID ?: "", userID)
                    .subscribe(Consumer {
                        (activity as? MainActivity)?.snackbarContainer?.let { it1 ->
                            PiloterrSnackbar.showSnackbar(it1,
                                    String.format(getString(R.string.removed_member), displayName), PiloterrSnackbar.SnackbarDisplayType.NORMAL)
                        }
                    }, RxErrorHandler.handleEmptyError())
            activity?.dismissKeyboard()
        }
